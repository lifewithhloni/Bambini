...
import { getBusinessVerificationSubmissionDetail } from "@/server/business/verification/adminBusinessVerifications";
import { ReviewActions } from "./ReviewActions";

export const dynamic = "force-dynamic";

export default async function AdminBusinessVerificationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const submission = await getBusinessVerificationSubmissionDetail(id);
  if (!submission) notFound();

  return (
    <div className="mx-auto flex w-full max-w-md flex-col gap-4 px-4 py-8 sm:py-12">
      <Link href="/admin/business-verifications" className="text-sm text-brand-muted hover:underline">
        ← Pending business verifications
      </Link>

      <h1 className="text-xl font-semibold text-brand-ink">{submission.businessName ?? "Unknown business"}</h1>
      ...
      <div className="flex flex-col gap-2 rounded-lg border border-brand-border bg-white p-4 text-sm">
        <div className="flex items-center justify-between">
          ...
          <span className="text-brand-ink">{submission.documentType}</span>
        </div>
        <div className="flex items-center justify-between">
