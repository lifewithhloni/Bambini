...
import { reviewBusinessVerification, type ReviewBusinessActionState } from "@/server/business/verification/reviewBusinessVerificationAction";
...
export function ReviewActions({ submissionId }: { submissionId: string }) {
  ...
  const approveAction = async (prev: ReviewBusinessActionState, formData: FormData) => {
    const result = await reviewBusinessVerification(submissionId, "verified", prev, formData);
    if (!result) router.push("/admin/business-verifications");
    return result;
  };
  const rejectAction = async (prev: ReviewBusinessActionState, formData: FormData) => {
    const result = await reviewBusinessVerification(submissionId, "rejected", prev, formData);
    if (!result) router.push("/admin/business-verifications");
    return result;
  };

  const [approveState, approveFormAction] = useActionState<ReviewBusinessActionState, FormData>(approveAction, null);
  const [rejectState, rejectFormAction] = useActionState<ReviewBusinessActionState, FormData>(rejectAction, null);
  ...
  return (
    <div className="flex flex-col gap-2 rounded-lg border border-brand-border bg-white p-4">
      <label htmlFor="notes" className="text-sm font-medium text-brand-ink">
        Review notes <span className="text-brand-muted">(shown to the business if rejected)</span>
      </label>
      <textarea
        id="notes"
        name="notes"
